...
export const waifu = {
    ...
    run: async(ctx: Context) => {
        if (!ctx.guild?.waifuArr)
            ctx.guild!.waifuArr = [];

        let emotes: Array<string> = [
            '<:yep:563113732872339456>', '<a:wink:667936075024564241>', '<:TranceGirlGasm:564557763024388097>',
            '<:blushu:458688271917121537>', '<a:abitch_aww:563514404637769728>', '<:yeah:506525240986173441>',
...
async function randomImage(arr: Array<string>, images: Array<string>, failed: number, ctx: Context) {
    let ran = await ctx.client.commandClient!.checkImage(images[Math.floor(Math.random() * images.length)]);
    if (ran === '') {
    if (failed === 3) {
        ctx.reply('I failed to get an image 3 times in a row. I\'m sorry <:sadness:405061263362752523>');
        return;
    }
    failed++;
